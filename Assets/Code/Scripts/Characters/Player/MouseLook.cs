...
{
    [Header("Mouse Look")]
    [SerializeField] float _sensitivityX = 0.3f;
    [SerializeField] float _sensitivityY = 0.3f;
    ...
    private void HandleMouseLook()
    {
        ...
        _rotationX -= _mouseY * _sensitivityY * (Screen.width/64) * Time.deltaTime;
        _rotationX = Mathf.Clamp(_rotationX, -_xClamp, _xClamp);
        _playerCamera.transform.localRotation = Quaternion.Euler(_rotationX, 0, 0);

        // looking left and right (rotates game object)
        transform.rotation *= Quaternion.Euler(0, _mouseX * _sensitivityX * (Screen.width/64) * Time.deltaTime, 0);
    }
}
